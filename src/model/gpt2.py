...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        y = F.scaled_dot_product_attention(q, k, v, is_causal=True) # Call flash attention in Pytorch
...
class MLP(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.gelu = nn.GELU(approximate="tanh") # Why use approximate=tanh => Beucase at the time, the extact version was slow => Created an approx version with tanh that was pciked by GPT2
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x + self.attn(self.ln_1(x)) # We can see the attention part as pooling/agregation. We try to find a link between tokens => Importance in tokens  => Reduce
...
class GPT(nn.Module):
    ...
    def forward(self, idx, targets=None):
        ...
        tok_emb = self.transformer.wte(idx) # token emebeddings of shape (B, T, n_embd)
        x = tok_emb + pos_emb
    ...
    def from_pretrained(cls, model_type):
        ...
        # create a from-scratch initialized mini GPT model
